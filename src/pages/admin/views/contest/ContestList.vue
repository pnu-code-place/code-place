<template>
  <div class="view">
    <Panel :title="$t('m.Contest_List_Page_Title')">
      <div slot="header">
        <el-input
          v-model="keyword"
      ...
      <el-table
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <p>{{$t('m.Contest_List_Start_Time')}}: {{props.row.start_time | localtime }}</p>
            <p>{{$t('m.Contest_List_End_Time')}}: {{props.row.end_time | localtime }}</p>
            <p>{{$t('m.Contest_List_Create_Time')}}: {{props.row.create_time | localtime}}</p>
            <p>{{$t('m.Contest_List_Creator')}}: {{props.row.created_by.username}}</p>
          </template>
        </el-table-column>
        <el-table-column
          prop="id"
          width="80"
          :label="$t('m.Contest_List_Table_Id')">
        </el-table-column>
        <el-table-column
          prop="title"
          :label="$t('m.Contest_List_Table_Title')">
        </el-table-column>
        <el-table-column
          :label="$t('m.Contest_List_Table_Rule_Type')"
          width="130">
        ...
        <el-table-column
          :label="$t('m.Contest_List_Table_Contest_Type')"
          width="180">
          <template slot-scope="scope">
        ...
        <el-table-column
          width="130"
          :label="$t('m.Contest_List_Table_Status')">
          <template slot-scope="scope">
            <el-tag
        ...
        <el-table-column
          width="100"
          :label="$t('m.Contest_List_Table_Visible')">
          <template slot-scope="scope">
    ...
    <el-dialog :title="$t('m.Contest_List_DownloadModal_Title')"
               width="30%"
               :visible.sync="downloadDialogVisible">
      <el-switch v-model="excludeAdmin" :active-text="$t('m.Contest_List_DownloadModal_Exclude')"></el-switch>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="downloadSubmissions">{{$t('m.Contest_List_DownloadModal_Execute')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
