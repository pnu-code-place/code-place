<template>
  <main>
    <div class="contents" v-if="post">
      ...
      <div class="editor-wrapper" :class="{ 'is-focused': editor && editor.focused }" v-if="isEditing">
        <div class="editor-menubar" v-if="editor">
          <!-- 진하게 -->
          <div class="menubar__button" :class="{ 'is-active': editor.isActive.bold() }" @click="editor.commands.bold()">
          ...
          <!-- 인라인 코드 -->
          <div class="menubar__button" :class="{ 'is-active': editor.isActive.code() }" @click="editor.commands.code()">
...
export default {
  ...
  methods: {
    ...
    highlightCodeBlocks() {
      // 본문 영역 내의 모든 pre code 태그를 찾음
      const blocks = this.$el.querySelectorAll('.post-content pre code');
