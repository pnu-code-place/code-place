...
export default {
  ...
  computed: {
    ...
    maxScore() {
      return Math.max(
        ...Object.keys(this.fieldInfo).map((key) => this.fieldInfo[key].score),
      )
    },
    graphData() {
      // 각 영역별로 점수를 배열로 만들어서 반환
      return Object.keys(this.fieldInfo).map(
        (key) => this.fieldInfo[key].score / this.maxScore,
      )
    },
    tooltipFormatter() {
        ...
        <tbody>
          ...
          >
            ...
            <td class="field-score">
              ...
              <span class="ratio"
                >({{ (category.ranking_percent * 100).toFixed(1) }}%)</span
              >
            </td>
