  ...
  <div class="view">
    <Panel :title="$t('m.User_Statistic')">
      <el-row type="flex">
        <el-col
          :span="6"
          style="border-right: 1px solid #eeeeee; padding: 10px"
        >
          <span
            style="
              color: #5c6773;
          ...
          <ul>
            <li>
              <span style="color: #457dd3; font-weight: 800; font-size: small"
                >총 사용자 수 {{ this.stat.all_users }}명</span
              >
            </li>
            <li>
              <span style="color: #5c6773; font-weight: 800; font-size: small"
                >최고 관리자 {{ this.stat.super_admins }}명</span
              >
            </li>
            <li>
              <span style="color: #5c6773; font-weight: 800; font-size: small"
                >관리자 {{ this.stat.admins }}명</span
              >
            </li>
            <li>
              <span style="color: #5c6773; font-weight: 800; font-size: small"
                >일반 사용자 {{ this.stat.regular_users }}명</span
              >
            </li>
          </ul>
        </el-col>
        <el-col
          ...
          style="border-right: 1px solid #eeeeee; padding: 10px"
        >
          <span style="color: #5c6773; font-weight: 800; font-size: medium"
            >학과별 가입자 수 현황</span
          >
          <div
            style="
    ...
    <Panel :title="$t('m.Generate_User')">
      <el-form :model="formGenerateUser" ref="formGenerateUser">
        <el-row style="margin-bottom: 10px">
          <span style="color: #5c6773; font-weight: 800; font-size: medium">{{
            $t("m.Create_User_Table_Option_School")
          }}</span>
        </el-row>
        <el-row type="flex">
        ...
        <el-row style="margin-bottom: 10px">
          <span style="color: #5c6773; font-weight: 800; font-size: medium">{{
            $t("m.Create_User_Table_Option_Account")
          }}</span>
        </el-row>
        <el-row type="flex">
          <el-col :span="4" style="margin-right: 15px">
            <el-form-item
              :label="$t('m.Create_User_Table_Prefix')"
              prop="prefix"
              required
            >
              <el-input
                v-model="formGenerateUser.prefix"
                :placeholder="$t('m.Create_User_Table_Prefix')"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="4" style="margin-right: 15px">
            <el-form-item
              :label="$t('m.Create_User_Table_Number_Of_Mock')"
              required
            >
              <el-input-number
                v-model="formGenerateUser.num_of_mock"
                style="width: 100%"
              ></el-input-number>
            </el-form-item>
          </el-col>
        ...
        <el-row style="margin-bottom: 10px">
          <span style="color: #6d88b6">
            {{ $t("m.Create_User_Table_Notice") }}
          </span>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    getUserList(page) {
      ...
      api
        .getUserList((page - 1) * this.pageSize, this.pageSize, this.query)
        .then(
          (res) => {
        ...
        api
          ...
          .then((res) => {
            ...
            utils.downloadFile(url).then(() => {
              this.$alert(
                this.$t("m.Create_User_Modal_Content"),
                this.$t("m.Create_User_Modal_Title"),
              )
            })
            this.getUserList(1)
          })
  ...
  computed: {
    ...
    filteredDepartmentListForDummy() {
      ...
      return this.departmentList.filter(
        (item) => item.college_id === this.formGenerateUser.college,
      )
    },
