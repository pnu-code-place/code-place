...
UploadFile = (function (_super) {
  ...
  UploadFile.prototype._initUpload = function () {
    this.input = $("<input />", {
      type: "file",
      style:
        "position:absolute;top:0;right:0;height:100%;width:100%;opacity:0;filter:alpha(opacity=0);cursor:pointer;",
    }).prependTo(this.el)
    var _this = this
    this.el
      .on("click mousedown", "input[type=file]", function (e) {
        ...
        $.ajax({
          ...
          .done(function (res) {
            if (!res.success) {
              ...
              let link =
                '<a target="_blank" className="simditor-attach-link" href="' +
                res.file_path +
                '">' +
                res.file_name +
                "</a>"
