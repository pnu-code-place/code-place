...
이미 `RedisSentinel/redis-sentinel` 이름으로 배포된 클러스터는 새 `RedisSentinel/redis` 적용 후 기존 CR을 삭제해야 합니다. `kubectl apply -k`는 이름이 바뀐 리소스를 자동으로 삭제하지 않습니다.

```sh
kubectl delete redissentinel redis-sentinel -n code-place-dev --ignore-not-found
kubectl delete redissentinel redis-sentinel -n code-place-prod --ignore-not-found
```
