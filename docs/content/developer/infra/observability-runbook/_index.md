...
- Tempo는 단일 replica와 Longhorn RWO PVC 조합이므로 Deployment strategy가 `Recreate`여야 합니다. RollingUpdate로 바뀌면 업그레이드 중 새 Pod와 기존 Pod가 `tempo-data` PVC를 두고 경합해 rollout이 멈출 수 있습니다.
...
up{namespace="monitoring", service=~"blackbox-exporter|kubernetes-event-exporter|otel-collector|tempo|kube-prometheus-stack-grafana|loki|loki-canary|alloy"}
...
### CodePlaceDeploymentUnavailable / CodePlaceDeploymentRolloutStuck / CodePlacePDBAtRisk
...
kube_poddisruptionbudget_status_current_healthy{namespace="<namespace>", poddisruptionbudget=~"backend|frontend"}
kube_poddisruptionbudget_status_desired_healthy{namespace="<namespace>", poddisruptionbudget=~"backend|frontend"}
...
- PDB current healthy가 desired healthy 이하이면 node drain/upgrade 전에 backend/frontend replica readiness를 먼저 복구합니다.
...
### PostgresCollectorError / PostgresHADegraded / PostgresReplicaUnavailable / RedisReplicaUnavailable / RedisMemoryHigh / RedisConnectedClientsHigh / RedisRejectedConnections
...
- PostgreSQL connection/long transaction/lock wait는 backend `/metrics`에서 직접 DB 조회하지 않습니다. 필요하면 CNPG exporter에 동일 의미의 metric이 있는지 운영 클러스터에서 먼저 확인한 뒤, 확정된 exporter metric만 dashboard/rule에 추가합니다.
