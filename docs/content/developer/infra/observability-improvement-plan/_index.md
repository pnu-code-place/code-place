...
- **User Path:** synthetic public HTTPS probe, Traefik/Ingress, frontend nginx, backend Django API.
...
- **AI Hint Path:** backend AI hint API, vLLM OpenAI-compatible server, vLLM HF cache PVC.
...
공개 사용자 경로는 Blackbox exporter 기반 synthetic probe로 별도 확인합니다. 이 probe는 `https://k3s.code-place-dev.site/`, `https://code.pusan.ac.kr/`를 30초 간격으로 호출해 DNS, Traefik TLS termination, frontend nginx, 기본 라우팅을 함께 검증합니다.
...
- `public-endpoint-probes.yaml`: dev/prod 공개 HTTPS URL Probe, interval 30s.
...
- `PublicEndpointDown`: prod 공개 HTTPS endpoint probe 실패 1분 지속.
...
- `PublicEndpointDown`: dev 공개 HTTPS endpoint probe 실패 2분 지속.
...
9. Grafana의 `CodePlace Overview` dashboard에서 request rate, 5xx, latency, submission status, waiting queue, judge heartbeat, Celery task throughput/runtime, Pod readiness/restart, CPU/memory, PVC, PostgreSQL/Redis readiness panel을 확인합니다.
10. Grafana의 `CodePlace Logs` dashboard에서 Loki ready, Alloy node coverage, Loki PVC usage, ingress/frontend 4xx/5xx, 최근 backend error, judge/celery log panel을 확인합니다.
11. Grafana의 `CodePlace Logs` dashboard에서 `request_id` 변수에 실제 응답 header 또는 JSON log의 request ID를 넣고 해당 요청 로그가 좁혀지는지 확인합니다.
12. Grafana의 `CodePlace Traces` dashboard에서 Tempo ready, OTel Collector ready, trace receive/export rate, Tempo PVC usage를 확인합니다.
13. Grafana의 `CodePlace Storage` dashboard에서 Longhorn manager, volume robustness, node/disk usage panel이 비어 있지 않은지 확인합니다.
14. Grafana의 `CodePlace AI Inference` dashboard에서 vLLM scrape, pod ready, waiting/running requests, KV cache, p95 latency, token throughput, HF cache PVC panel이 비어 있지 않은지 확인합니다.
15. Grafana Explore에서 `Loki` datasource를 선택하고 `{namespace="code-place-dev"}` 쿼리가 로그를 반환하는지 확인합니다.
16. Grafana Explore에서 `Tempo` datasource를 선택하고 `service.name=codeplace-backend` 또는 `service.name=codeplace-celery` trace가 조회되는지 확인합니다.
17. test alert 또는 임시 rule로 P0 webhook 수신 시간이 1분 이내인지 확인합니다.

운영 적용 전제는 다음과 같습니다.

- `/metrics`는 cluster 내부 scrape 전용이며 외부 Ingress에 노출하지 않습니다.
- 공개 URL synthetic probe는 monitoring namespace 내부에서 외부 도메인으로 나가는 HTTPS 요청입니다. DNS, outbound network, TLS, Traefik/frontend 경로를 함께 검증하지만 브라우저 JS runtime error를 대체하지는 않습니다.
