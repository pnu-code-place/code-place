...
- `PostgresUnavailable`: PostgreSQL Pod not ready 또는 exporter target down 1분 지속.
- `RedisUnavailable`: Redis Pod not ready 또는 exporter target down 1분 지속.
...
6. Grafana의 `CodePlace Overview` dashboard에서 request rate, 5xx, latency, submission/judge outcome, waiting queue, judge heartbeat panel을 확인합니다.
