...
7. Prometheus target에서 `backend`, `longhorn`, `blackbox-exporter`, `codeplace-public-http-dev`, `codeplace-public-http-prod`가 healthy인지 확인합니다.
...
14. Grafana Explore에서 `Loki` datasource를 선택하고 `{namespace="code-place-dev"}` 쿼리가 로그를 반환하는지 확인합니다.
15. Grafana Explore에서 `Tempo` datasource를 선택하고 `service.name=codeplace-backend` 또는 `service.name=codeplace-celery` trace가 조회되는지 확인합니다.
16. test alert 또는 임시 rule로 P0 webhook 수신 시간이 1분 이내인지 확인합니다.
